...
Na versão de prompt `gemini-superficie-v6`, os testes conservam a proteção introduzida no v5: códigos internos e nomes de arquivos ou botões não são enviados ao modelo. Os 18 perfis visuais precisam permanecer distintos, para que mudar o modelo de uma lesão continue alterando o prompt e seu hash. Mesmo em elementos idênticos e sobrepostos, a ordem e as autorizações são diferenciadas; trocar a seleção depois da preparação impede a chamada. Esses testes verificam o pedido e as proteções do fluxo; não avaliam a qualidade visual de uma geração.
