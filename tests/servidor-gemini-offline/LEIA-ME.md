...
Na avaliação do Gemini Pro, a preparação, o envio e os registros precisam identificar `gemini-3-pro-image` com imagem `2K`, sem o parâmetro `thinkingConfig: minimal` do Lite. Abas com a versão anterior `refinamento-v2` param antes de reservar cota. Uma indisponibilidade do Pro não pode repetir a solicitação nem tentar o Lite automaticamente.
