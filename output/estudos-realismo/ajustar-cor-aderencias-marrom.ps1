...
public static class CorAderenciasMarrom {
            ...
            for (int y = 0; y < entrada.Height; y++) {
                for (int x = 0; x < entrada.Width; x++) {
                    ...
                    saida.SetPixel(x, y, Color.FromArgb(cor.A,
                        (int)Math.Round(cor.R * 0.52),
                        (int)Math.Round(cor.G * 0.40),
                        (int)Math.Round(cor.B * 0.28)));
                }
            }
...
foreach ($numero in 1,2,3,4) {
    ...
    $destino = Join-Path $pastaLesoes "aderencia-$numero-marrom-escuro.png"
    [CorAderenciasMarrom]::Aplicar($origem, $destino)
